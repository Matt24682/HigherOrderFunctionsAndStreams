...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual =input.map(integer -> integer + 1).collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(string -> string.charAt(1) % 2 == 0).collect(Collectors.joining());
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(revision -> instant.isAfter(revision.timestamp)).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.map(revision -> revision.user).filter(whitelist::contains).count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(revision -> !revision.user.toUpperCase(Locale.ROOT).contains("BOT")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.map(revision -> revision.timestamp).filter(time -> time.atZone(ZoneId.of("UTC")).getMonth().equals(Month.FEBRUARY)).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.collect(Collectors.groupingBy(revision -> revision.timestamp.atZone(ZoneId.of("UTC")).getMonth(), Collectors.counting()))
                .entrySet().stream().max(Map.Entry.comparingByValue(Long::compareTo)).orElseThrow().getKey();
